var Application = function() {
  ...
  var dateRangePicker = function() {
    ...
    $('input[name="daterange"]').on('apply.daterangepicker', function(ev, picker) {
      //console.log(picker.startDate.toDate().getTime());
      //console.log(picker.endDate.toDate().getTime());

      var url = [
        "/api/v1/data/chart/aggregate",
        picker.startDate.toDate().getTime(),
        picker.endDate.toDate().getTime()
      ].join('/');
      var canvas = picker.element.parent().parent().parent().parent().parent().find('canvas');

      drawAreaChart(url, canvas.attr('id'));
    });
  };
